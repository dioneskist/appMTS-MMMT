...
class TelaTreinoAB(Screen):
    ...
    def show_smile(self, number):
        ...
        if int(number) == 2:
            ...
            self.apagar_widget_id = self.ids._smile2
            self.desaparecer_smile()
        if int(number) == 3:
            self.ids._smile3.source = 'figuras/smile.png'
            self.apagar_widget_id = self.ids._smile3
            self.desaparecer_smile()
    ...
    def incrementa_erro(self):
        ...
        self.manager.latencia_erro_str = "Latencia erro: {0:.2f}".format(Clock.get_time() - self.manager.latencia) + ' segundos'

    def incrementa_acerto(self):
        logging.debug(
            'TelaTreinoAB.incrementa_acerto: incrementando acertos de {} para {}'.format(self.acertos, self.acertos + 1))
        self.acertos += 1
        self.manager.acertos_total += 1
        self.manager.acertos_total_str = 'Acertos:  '+ str(self.manager.acertos_total)
        self.manager.latencia_acerto_str = "Latencia acerto: {0:.2f}".format(Clock.get_time() - self.manager.latencia) + ' segundos'
        if self.acertos == 3:
    ...
    def troca_tela(self, delta):
        ...
        logging.debug('TelaTreinoAB.troca_tela: trocando tela de {} para {}'.format(self.manager.current, proxima_tela))
